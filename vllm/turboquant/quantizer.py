...
class TurboQuantizer(nn.Module):
    ...
    def attention_scores(
        ...
        qjl_ip = torch.matmul(q_proj, signs_float.transpose(-2, -1))
    ...
    def pack_cache(
        ...
        packed_mse = self._pack_bits(flat_idx, mse_bits)

        flat_signs = ((signs.reshape(-1, d) + 1) // 2).to(torch.uint8)
    ...
    def unpack_cache(self, packed: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        signs = (signs_01.to(torch.int8) * 2 - 1)
